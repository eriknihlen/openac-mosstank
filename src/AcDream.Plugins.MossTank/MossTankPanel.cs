...
internal sealed partial class MossTankPanel : IBuffRuleHost, IDisposable
{
    ...
    private readonly InventoryCountController _inventoryCount;
    // Built with the UB settings, which they read live; see InitializeUbSettings.
    private VendorTradeController _vendorTrade = null!;
    private ExperienceSpendController _experienceSpend = null!;
    private EquipProfileController _equipProfile = null!;
    ...
    /// <summary>The colour an ordinary waypoint row draws in.</summary>
    private const uint RouteWaypointColor = 0xE8DEC3u;

    /// <summary>The colour a once route's spent waypoint draws in.</summary>
    private const uint RouteWaypointConsumedColor = 0xC03838u;
    ...
    internal MossTankPanel(IPluginHost host, IVtankGameInfoTransport? gameInfoTransport)
    {
        ...
        _readScroll = new ReadScrollController(
            ...
            _inventorySettings.Loot,
            _loot);
        _profileGive = new ProfileGiveController(
            host,
            _lootProfiles,
            _inventorySettings);
        // The counter and the looter read the same appraisal slot, so the
        // counter is told how to ask whether the looter is using it.
        _inventoryCount = new InventoryCountController(
            host,
            _lootProfiles,
            () => _loot.IsIdentifying);
        ...
        _meta = new MetaEngine(
            ...
            new MetaServices
            {
                ...
                // The settings page switch; the catalogue is built after the
                // engine, so it is looked up on each pass rather than captured.
    ...
    public Action ShowUb => () => SelectTab(TankTab.Ub);
    public Action ShowUbSettings => () => SelectTab(TankTab.UbSettings);
    ...
    public bool UbSelected => _activeTab == TankTab.Ub;
    public bool UbSettingsSelected => _activeTab == TankTab.UbSettings;
    ...
    /// <summary>
    /// A once route paints the points it has already run red, so the list
    /// shows how far the route has got without waiting for a reload. Every
    /// other mode, and every point the cursor has not reached, draws normally.
    /// </summary>
    public IReadOnlyList<uint> RouteWaypointColors
    {
        get
        {
            var colors = new uint[_routeRows.Count];
            for (int i = 0; i < colors.Length; i++)
            {
                colors[i] = _navigation.IsWaypointConsumed(i)
                    ? RouteWaypointConsumedColor
                    : RouteWaypointColor;
            }
            return colors;
        }
    ...
    private void AddSelectedComponentCore()
    {
        ...
        {
            _profileNotice = "That blacklist entry is already in the list.";
            return;
        }
    ...
    private void RegisterVtankExpressionFunctions()
    {
        ...
        functions.Register("vtmacroenabled", 0, 0, (_, _) =>
            ...
            "vtmacroenabled[]");
        // The three counts as numbers, so a meta can branch on the figure
        // instead of reading it back out of a chat line.
        functions.Register("vtcountitem", 1, 1, (_, args) =>
        {
            if (!_inventoryCount.TryCountByName(
                args[0].AsString("vtcountitem"),
                out InventoryCountTally tally,
                out string error))
            {
                throw new ExpressionEvaluationException("vtcountitem: " + error);
            }
            return ExpressionValue.Number(tally.Total);
        }, "vtcountitem[namepattern]");
        functions.Register("vtcountplayer", 1, 1, (_, args) =>
            ExpressionValue.Number(_inventoryCount.CountPlayersWithin(
                args[0].AsNumber("vtcountplayer"))),
            "vtcountplayer[range]");
        // -1 while there is no answer yet: the count is still waiting on an
        // appraisal, or the profile could not be read. Poll for a figure of
        // zero or more.
        functions.Register("vtcountprofile", 1, 1, (_, args) =>
            ExpressionValue.Number(_inventoryCount.ProfileCount(
                args[0].AsString("vtcountprofile"))),
            "vtcountprofile[lootprofile]");
    }
    ...
    private void EnsureCharacterProfile()
    {
        ...
        if (VtankProfileDirectory.CanonicalCharacterKey(characterName).Length == 0)
            return;
        BindUbSettingsToCharacter(characterName);
    ...
    private const string DeathRestoredNotice =
        "Buffing, Combat, Nav and Loot are back to their earlier values.";
    ...
    public void OnTick(double elapsedSeconds)
    {
        ...
        bool countOwnsAction = _inventoryCount.Tick(
            ...
            canAct: !_buffRule.IsBursting
                ...
                && !giveOwnsAction);
        bool vendorOwnsAction = _vendorTrade.Tick(
            elapsedSeconds,
            canAct: !_buffRule.IsBursting
                && !commandJumpOwnsAction
                && !giveOwnsAction
                && !countOwnsAction);
        bool equipOwnsAction = _equipProfile.Tick(
            elapsedSeconds,
            canAct: !_buffRule.IsBursting
                && !commandJumpOwnsAction
                && !giveOwnsAction
        ...
        // What the other clients on this computer cast is read on the frame
        // too, so the next pass already knows a debuff another character
        // landed instead of learning it a pass late and casting it again.
        _combat.ObserveRemoteCasts();
        _scheduler.ExternalSuspension = _prologueOwnsAction;
        ReportSchedulerSuspension(
            commandJumpOwnsAction,
            giveOwnsAction,
            countOwnsAction,
            vendorOwnsAction);
        ...
        _navigation.StepArmedMover(elapsedSeconds, NavigationSlotsAreClear());
    ...
    /// </summary>
    private void ReportSchedulerSuspension(
        bool commandJumpOwnsAction,
        bool giveOwnsAction,
        bool countOwnsAction,
        bool vendorOwnsAction)
    {
        string? cause = !_scheduler.IsRunning
            ...
            : commandJumpOwnsAction
                ...
                : giveOwnsAction
                    ...
                    : countOwnsAction
                        ? "a profile count owns the character"
                        : vendorOwnsAction
                            ? "a vendor run owns the character"
