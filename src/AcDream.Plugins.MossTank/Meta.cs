...
internal enum MetaConditionKind
{
    ...
    // The client's own notifications as edges. They are appended so the
    // numbers of the kinds above, which stored profiles carry, do not move.
    // A rule on one of these re-arms after each pass: an edge is an
    // occurrence, not a state, so its rule may fire once per occurrence
    // rather than once per state entry. The reference tool's pre-in-world
    // "login" event has no counterpart on this client, whose first
    // notification is the completed login; it maps here.
    LoginComplete,
    Logoff,
    PortalTransition,
    ItemUseCompleted,
...
internal sealed class MetaServices
{
    ...
    /// <summary>
    /// The settings page's switch for the event handlers. Off, a rule shaped
    /// like a handler (see <see cref="GameEventHandlers.IsHandler"/>) is
    /// skipped; every other rule, edge-conditioned or not, still runs.
    /// </summary>
    public Func<bool> GameEventHandlersEnabled { get; init; } = static () => true;
}
...
internal sealed class MetaEngine : IDisposable
{
    ...
    private bool _loginCompleteEdge;
    private bool _logoffEdge;
    private bool _portalTransitionEdge;
    private bool _itemUseEdge;
    private bool _containerOpenedEdge;
    private bool _containerClosedEdge;
    private bool _confirmationEdge;
    private readonly Action _onLoginComplete;
    private readonly Action _onLogoff;
    private readonly Action<string> _onLocalPlayerDied;
    private readonly Action<PluginPortalTransition> _onPortalTransition;
    private readonly Action<PluginItemUseCompletion> _onItemUseCompleted;
    private readonly Action<uint> _onContainerOpened;
    private readonly Action<uint> _onContainerClosed;
    private readonly Action<PluginConfirmation> _onConfirmationRequested;
        ...
        MetaServices? services = null)
    {
        ...
        // The client's notifications are latched as edges and consumed by
        // the next decision pass, the way the portal and death edges are.
        // Their fields go to the session variables at the moment they are
        // raised, so an expression in the handler reads what the event
        // carried; the login variables are the exception and are read when
        // the edge is evaluated, because the plugin's own session reset
        // (which clears the session variables) runs after the client's
        // login notification.
        _onLoginComplete = () => _loginCompleteEdge = true;
        _onLogoff = () =>
        {
            // There is no next pass in a session that is ending: the
            // client raises this before the teardown, so the pass runs now.
        ...
        _onLocalPlayerDied = message =>
        {
            SetEventVariable("deathmessage", ExpressionValue.String(message));
            _deathEdge = true;
        };
        _onPortalTransition = transition =>
        {
            // The edge is the arrival, not every report along the way.
            if (!transition.IsCompleted)
                return;
            SetEventVariable("destinationcell", ExpressionValue.Number(transition.DestinationCell));
            _portalTransitionEdge = true;
        };
        ...
        {
            SetEventVariable("contextid", ExpressionValue.Number(confirmation.ContextId));
            SetEventVariable("confirmtype", ExpressionValue.Number(confirmation.Type));
            SetEventVariable("text", ExpressionValue.String(confirmation.Text));
            _confirmationEdge = true;
        };
        IEvents events = host.Events;
        events.LoginComplete += _onLoginComplete;
        events.Logoff += _onLogoff;
        events.LocalPlayerDied += _onLocalPlayerDied;
        events.PortalTransition += _onPortalTransition;
        events.ItemUseCompleted += _onItemUseCompleted;
        events.ContainerOpened += _onContainerOpened;
        events.ContainerClosed += _onContainerClosed;
        events.ConfirmationRequested += _onConfirmationRequested;
    }
    ...
    public void ResetSession()
    {
        ...
        _portalExited = false;
        _deathEdge = false;
        // The login-complete edge is the one that survives: the client
        // raises it before the plugin's own new-session reset runs, and a
        // reset that cleared it would lose every login handler.
        ClearEventEdges(keepLoginComplete: true);
        _watchdog = null;
        _status = "Meta disabled.";
    }
    ...
    public void OnTick(double elapsedSeconds)
    {
        ...
        _portalExited = false;
        _deathEdge = false;
        ClearEventEdges(keepLoginComplete: false);
        _chatBatch.Clear();
    }
    ...
    public void EvaluatePass()
    {
        ...
        bool handlersEnabled = _services.GameEventHandlersEnabled();
        try
        {
            RunPass(rules, handlersEnabled);
        }
        finally
        {
            // An edge is an occurrence: its rule may fire again on the next
            // one, so it is re-armed once the pass is over. The pass, not
            // the rule, decides this, so a transition mid-pass re-arms too.
            foreach (MetaRule rule in rules)
            {
                if (GameEventHandlers.IsEventEdge(rule.Condition.Kind))
                    _fired.Remove(rule.Id);
            }
        }
    }
    ...
    private bool EvaluateCondition(MetaCondition condition) => condition.Kind switch
    {
        ...
        MetaConditionKind.ItemUseCompleted => _itemUseEdge,
        MetaConditionKind.ContainerOpened => _containerOpenedEdge,
        MetaConditionKind.ContainerClosed => _containerClosedEdge,
    ...
    /// <summary>
    /// The event's fields for expressions, named the way the reference
    /// tool names them: <c>gevt_</c> and the field, in lower case. They go
    /// through the same session-variable path a chat capture's groups do.
    /// </summary>
    private void SetEventVariable(string name, ExpressionValue value) =>
        _expressions.State.Set(ExpressionVariableScope.Session, "gevt_" + name, value);
    ...
    private void ClearEventEdges(bool keepLoginComplete)
    {
        if (!keepLoginComplete)
            _loginCompleteEdge = false;
        _logoffEdge = false;
        _portalTransitionEdge = false;
        _itemUseEdge = false;
        _containerOpenedEdge = false;
        _containerClosedEdge = false;
        _confirmationEdge = false;
    }
    ...
    private bool ExecuteAction(MetaAction action)
    {
        switch (action.Kind)
        {
            ...
            case MetaActionKind.CallMetaState:
                if (_callStack.Count >= MaximumCallDepth)
                {
                    DisableWithError("Meta Error: too many nested calls; check for a call loop.");
                    return false;
                }
            ...
            case MetaActionKind.ReturnFromCall:
                ...
                {
                    DisableWithError("Meta Error: a return with no matching call.");
                    return false;
                }
    ...
    /// <summary>
    /// What a death cost, read from the server's own line about it: the
    /// death notification names the killer, the chat line names the loss,
    /// and a handler wants both.
    /// </summary>
    private void CaptureDroppedItems(string text)
    {
        const string lost = "You've lost ";
        if (text.StartsWith(lost, StringComparison.Ordinal))
            SetEventVariable("droppeditems", ExpressionValue.String(text[lost.Length..]));
        else if (text.StartsWith("You have retained all your items.", StringComparison.Ordinal)
            || text.StartsWith("You are enveloped in a feeling of warmth", StringComparison.Ordinal))
        {
            SetEventVariable("droppeditems", ExpressionValue.String(string.Empty));
        }
    }
    ...
    {
        // The backtick is the quote, so an item name only has to escape a
        // backslash and a backtick; an apostrophe travels as itself.
