...
internal sealed partial class MossTankPanel : IMacroRuleProvider
{
    ...
    private bool NavigationLocksAreClear() =>
        !_navigationWaitsOnCorpseId && NavigationSlotsAreClear();

    /// <summary>
    /// The three slots alone, without the per-pass corpse-id latch, which is
    /// meaningless between passes. This is what the frame driver asks before
    /// it steps the armed mover: the rule pass is not asked at all while
    /// another owner holds the character, so the slots are the only thing
    /// that can stop the route on a frame.
    /// </summary>
    internal bool NavigationSlotsAreClear() =>
