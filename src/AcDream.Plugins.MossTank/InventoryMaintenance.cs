...
internal sealed class InventorySettings
{
    ...
    /// <summary>
    /// How far off, flat along the ground, a character may stand and still be
    /// given items. A hand-over is a face-to-face thing, so a target across a
    /// courtyard is refused rather than walked to.
    /// </summary>
    public double GiveRangeMeters { get; set; } = 15d;

    /// <summary>
    /// How long to wait between one hand-over and the next. Zero is no wait,
    /// which is what a server that keeps up wants.
    /// </summary>
    ...
    /// <summary>
    /// How many items a run may write off before it stops altogether. A run
    /// that cannot give anything should say so rather than work through the
    /// packs failing on every one.
    /// </summary>
    public int GiveFailureLimit { get; set; } = 3;
