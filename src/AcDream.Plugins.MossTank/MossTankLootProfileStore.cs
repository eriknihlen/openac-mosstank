...
internal sealed class MossTankLootProfileStore
{
    ...
    public bool TryLoadItemGiver(string? name, List<LootRule> target)
    {
        ArgumentNullException.ThrowIfNull(target);
        string normalized = name?.Trim() ?? string.Empty;
        if (normalized.Length == 0 || !VtankStorage.IsAvailable)
            return false;
        string? text = VtankStorage.ReadText(StorageLayout.ItemGiverProfileKey(normalized));
    ...
    /// <summary>
    /// One profile's rules as the text of a profile file, expressions and
    /// all, for a caller that keeps profile files somewhere of its own.
    /// </summary>
    internal static string SerializeRules(IReadOnlyList<LootRule> rules)
    {
        ArgumentNullException.ThrowIfNull(rules);
        var profile = new VtankLootProfile { Rules = rules.ToList() };
        AttachMossTankExpressions(profile);
        return VtankLootProfileSerializer.Write(profile);
    }

    /// <summary>
    /// Reads the rules out of the text of a profile file, expressions and
    /// all. False when the text is not a profile.
    /// </summary>
    internal static bool TryParseRules(string? text, List<LootRule> target)
    {
        ArgumentNullException.ThrowIfNull(target);
        if (text is null
            || !VtankLootProfileSerializer.TryRead(text, out VtankLootProfile profile, out _))
        {
            return false;
        }
        ApplyMossTankExpressions(profile);
