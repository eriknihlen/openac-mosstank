...
try {
    ...
    # The license and the third-party notices travel with the binary. LICENSE
    # has no extension and the installer's allowlist would refuse it, so it
    # ships as LICENSE.txt.
    $repoRoot = Split-Path -Parent $PSScriptRoot
    Copy-Item -LiteralPath (Join-Path $repoRoot 'LICENSE') -Destination (Join-Path $staging 'LICENSE.txt')
    Copy-Item -LiteralPath (Join-Path $repoRoot 'NOTICE.md') -Destination (Join-Path $staging 'NOTICE.md')
